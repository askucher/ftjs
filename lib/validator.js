...
module.exports = function(source){
  var modules, findType, lookupType, lookupInvokeFunctionType, lookupDiscriminationType, getExpectedTypes, validateComplexType, parseInvoke, invokeRegularExpression, invokeOperationWithValue, invokeFunction, validateValue, validateExpression, validateType, validate;
  ...
  validate.syntaxCheck = function(it){
    var result;
    result = p.map(function(it){
      return "'" + it[0] + "': { " + it[1] + " }";
    })(
    p.filter(toString$.call(it[1]).slice(8, -1) === 'String')(
    p.objToPairs(
    modules)));
    if (result.length === 0) {
      return true;
