...
module.exports = function(source){
  var registry, findType, lookupType, lookupInvokeFunctionType, lookupDiscriminationType, getExpectedTypes, validateComplexType, parseInvoke, invokeRegularExpression, invokeOperationWithValue, invokeFunction, validateValue, validateExpression, validateType, validate;
  ...
  validateComplexType = function(scope, type, obj){
    var parts, nextType, currentType, validate, result;
    if (type.type !== 'Complex' && type.body.indexOf('.') > -1) {
      parts = type.body.split('.');
      nextType = findType(parts[0], parts[1]);
      return validateComplexType(parts[0], nextType, obj);
    }
  ...
  validateValue = curry$(function(scope, obj, type){
    ...
    case 'Internal.Extended':
      ...
      apply = function(invoke){
        var func, appliedParams, applyParam;
        func = type.basetype.extensions.filter(function(it){
    ...
    case 'ArrayType':
      ...
      if (type.body.indexOf('.') > -1) {
        parts = type.body.split('.');
        found = findType(parts[0], parts[1]);
        return processArrayType(parts[0], found);
      } else {
