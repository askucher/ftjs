...
module.exports = (pattern = /(#.+)|(([A-Z][a-zA-Z0-9]+)(\s[a-z][a-zA-Z0-9]+)+\s+:\s+(.+))|(([A-Z][a-zA-Z0-9]+)\s+:\s+(.+))|(([a-z][a-zA-Z0-9]+)\s+\:\s+(.+))|(-{3,})|(([A-Z][a-zA-Z0-9]+)\.{3})|(([A-Z][a-zA-Z0-9]+))/, functionParams = function(str){
  return str.match(/[A-Z][a-zA-Z0-9]+\s([^:]+)/)[1].trim().split(/\s+/);
}, bodyPattern = /(\/.+)|([^\|]+\|.+)|.+([A-Z][a-zA-Z0-9]+\([^(]+\))|(\[[A-Z][a-zA-Z0-9]+\])|(\@.+)|(\"[^"]+\")|(-?[0-9]+)|([A-Z][a-zA-Z0-9]+\.[A-Z][a-zA-Z0-9]+)|([A-Z][a-zA-Z0-9]+)/, readDiscrimination = function(str){
  ...
  if (toString$.call(input).slice(8, -1) !== 'Array') {
    return ["Naming error: " + toString$.call(input).slice(8, -1) + " is not supported"];
  }
  if (input.length === 0) {
    return [];
  }
  return tokenize(input[0]).concat(naming(p.tail(input)));
}, read = function(input){
  if (toString$.call(input).slice(8, -1) === 'Buffer') {
  ...
  return [input[0].match(pattern)].concat(read(p.tail(input)));
}, checkers = [
  {
    curr: 'Field',
